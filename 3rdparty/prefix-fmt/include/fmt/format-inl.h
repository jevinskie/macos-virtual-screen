...
FMT_FUNC auto write_loc(appender out, loc_value value, const format_specs &specs,
                        locale_ref loc) -> bool {
...
template <typename F, FMT_ENABLE_IF(sizeof(F::_p) != 0)> auto get_file(F *f, int) -> apple_file<F> {
    return f;
}
