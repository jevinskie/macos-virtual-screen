...
template <typename T> FMT_CONSTEXPR auto copy(const T *begin, const T *end, T *out) -> T * {
...
FMT_CONSTEXPR auto make_arg(T &val) -> value<Context> {
    ...
    enum {
        formattable_char = !std::is_same < arg_type,
        unformattable_char > ::value
    };
    ...
    enum {
        formattable_pointer = !std::is_same < arg_type,
        unformattable_pointer > ::value
    };
    static_assert(formattable_pointer, "Formatting of non-void pointers is disallowed.");

    enum {
        formattable = !std::is_same < arg_type,
        unformattable > ::value
    };
#if defined(__cpp_if_constexpr)
...
public:
    ...
    /**
      ...
      \endrst
    */
    template <typename Visitor> FMT_CONSTEXPR auto visit(Visitor &&vis) -> decltype(vis(0)) {
        switch (type_) {
...
struct format_specs {
    ...
    constexpr format_specs()
        : width(0), precision(-1), type(presentation_type::none), align(align::none),
          sign(sign::none), upper(false), alt(false), localized(false) {}
};
...
template <typename Char>
FMT_CONSTEXPR auto
parse_format_specs(const Char *begin, const Char *end, dynamic_format_specs<Char> &specs,
...
template <typename... T> FMT_INLINE void print(format_string<T...> fmt, T &&...args) {
    ...
    return detail::is_locking<T...>() ? vprint(fmt, vargs) : vprint_locked(stdout, fmt, vargs);
}
...
template <typename... T> FMT_INLINE void print(FILE *f, format_string<T...> fmt, T &&...args) {
    ...
    return detail::is_locking<T...>() ? vprint(f, fmt, vargs) : vprint_locked(f, fmt, vargs);
}
